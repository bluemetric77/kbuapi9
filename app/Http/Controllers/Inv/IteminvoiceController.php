...
class IteminvoiceController extends Controller
{
    public function show(Request $request)
    {
        ...
        $query = ItemInvoice1::selectRaw("sysid, doc_number, ref_date, ref_document, order_document, partner_name,
                due_date, amount, discount1, discount2, payment_discount, tax,
                total, payment, unpaid, CONCAT(trans_code, '-', trans_series) AS voucher, sysid_jurnal,
                pool_code, warehouse_id, void_date, is_void, void_by, uuid_rec, IFNULL(doc_name, '') AS doc_name")
    ...
    public function destroy(Request $request)
    {
        ...
        $invoice = ItemInvoice1::selectRaw('doc_number, is_void, payment, sysid_jurnal')
            ->where('uuid_rec', $uuid)
            ->where('is_credit_note','0')
            ->first();
        ...
        $isPaid = DB::table('t_customer_account')
            ->where('ref_sysid', $invoice->sysid)
            ->whereRaw("doc_source='LPB' AND IFNULL(total_paid, 0) > 0")
            ->first();

        if ($isPaid) {
            return response()->error('', 501, 'Invoice pembelian tidak bisa diubah, invoice sudah ada pembayaran');
        }

        DB::beginTransaction();
        try {
            ItemInvoice1::where('sysid', $invoice->sysid)
                ->update([
                    'is_void' => 1,
                    'void_by' => PagesHelp::UserID($request),
                    'void_date' => now() // Use Laravel's now() for current timestamp
                ]);

            $inventoryResponse = Inventory::ItemCard($invoice->sysid, 'LPB', 'deleted', true, false, true);
            if (!$inventoryResponse['success']) {
                DB::rollback();
                return response()->error('', 501, $inventoryResponse['message']);
            }

            $journalResponse = Accounting::void_jurnal($invoice->sysid_jurnal, $request);
            if (!$journalResponse['state']) {
                DB::rollback();
                return response()->error('', 501, $journalResponse['message']);
            }

            $accountingConfig = Accounting::Config();
            Accounting::void_customer_account($invoice->sysid, 'LPB', $accountingConfig->ap_invoice);

            DB::commit();

            return response()->success('Success', [
                'sysid' => $invoice->sysid,
                'message' => "Pembatalan invoice barang berhasil"
            ]);

        } catch (Exception $e) {
            DB::rollback();
            return response()->error('', 501, $e->getMessage()); // Return the exception message
        }
    }
    ...
    public function post(Request $request){
        ...
        $invoice=ItemInvoice1::selectRaw('sysid,doc_number,ref_date,is_void,payment')->where('uuid_rec',$uuid)->first();
        ...
        try{
            ...
            if (!$invoice) {
                ...
                $invoice->doc_number= ItemInvoice1::GenerateNumber($header['pool_code'],$header['ref_date']);
                $opr='inserted';
            } else {
                $opr='updated';
                $ispaid=DB::table('t_customer_account')
               ...
               ItemInvoice1::cancel_order($sysid);
            ...
            ItemInvoice1::update_order($sysid);
            ...
            DB::UPDATE("UPDATE t_item_invoice1 a
                ...
                SET a.amount=b.amount,a.discount1=b.discount1,a.discount2=b.discount2,a.tax=b.tax,a.total=b.total,a.unpaid=b.total
                WHERE a.sysid=?",[$sysid,$sysid]);

            $respon=Inventory::ItemCard($sysid,'LPB',$opr,true,false);
            ...
            foreach($detail as $rec) {
                $itmPartner=ItemPartner::where('item_code', $rec['item_code'])
                ->where('partner_code',$invoice->partner_code)->first();

                if (!$itmPartner) {
                    $itmPartner= new ItemPartner();
                    $itmPartner->item_code    = $rec['item_code'];
                    $itmPartner->partner_code = $invoice->partner_code;
                }
                $itmPartner->mou_purchase    = $rec['mou_purchase'];
                $itmPartner->convertion      = $rec['convertion'];
                $itmPartner->mou_inventory   = $rec['mou_inventory'];
                $itmPartner->price           = $rec['purchase_price'];
                $itmPartner->prc_discount    = $rec['prc_discount1'];
                $itmPartner->prc_tax         = $rec['prc_tax'];
                $itmPartner->last_invoice_no = $invoice->doc_number;
                $itmPartner->last_purchase   = $invoice->ref_date;
                $itmPartner->save();
            }
            DB::commit();
            $respon=[
    ...
    public function print(Request $request){
        ...
        ->selectRaw("a.sysid,a.doc_number,a.ref_document,a.order_document,a.partner_name,a.ref_date,a.due_date,
            a.amount,a.discount1,a.tax,a.total as net_total,concat(a.trans_code,'-',a.trans_series) as voucher,a.pool_code,
            a.update_userid,a.update_timestamp,IFNULL(b.descriptions,'') as warehouse_name,c.user_name")
        ...
        $sign['user_sign']=isset($user->sign) ? $home.$user->sign :'';

            $detail=ItemInvoice2::from('t_item_invoice2 as a')
        ->selectRaw("a.line_no,a.item_code,a.descriptions,a.mou_purchase,a.qty_invoice,a.purchase_price,a.prc_discount1,
            a.prc_tax,a.total,IFNULL(b.part_number,'') as part_number")
        ->leftJoin('m_item as b','a.item_code','=','b.item_code')
...
public function upload_document(Request $request) {
    ...
    public function order(Request $request){
        ...
        $data=PurchaseOrder1::selectRaw("sysid,doc_number,CONCAT(doc_number,' - ',IFNULL(descriptions,'')) as descriptions,warehouse_id,pool_code")
    ...
    public function dtlorder(Request $request){
        $doc_number=$request->doc_number ?? '';
        $data=PurchaseOrder1::from('t_purchase_order1 as a')
        ->select('b.line_no','b.item_code','c.part_number','b.descriptions','b.mou_purchase','b.convertion','b.mou_warehouse',
    ...
    public static function build_jurnal($sysid,$request) {
        ...
        if ($data){
            ...
            $jurnal->fill([
                  ...
                  'transtype'=>'6',
                  'notes'=>'PEMBELIAN BARANG. '.$data->ref_document.' '.$data->partner_name
            ]);
            $jurnal->save();
            ...
            Journal2::insert([
                ...
                'line_memo' =>'Pembelian Barang. '.$data->ref_document.' '.$data->partner_name,
                'reference1'=>$data->doc_number,
                'reference2'=>$data->ref_document,
                'debit'     =>0,
